...
async function pack(input, output) {
  ...
  for(let i=0;i<sprites.length;i++) {
    ...
    const buffer=await sharp(raw,{raw:{width:s.width,height:s.height,channels:4}}).resize(width,height,{kernel:pixel?'nearest':'lanczos3'}).png().toBuffer();
    tiles.push({input:buffer,left:(i%4)*270+Math.floor((270-width)/2),top:Math.floor(i/4)*270+260-height});
  }
  await sharp({create:{width:1080,height:1080,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite(tiles).webp({quality:94,alphaQuality:100,lossless:pixel}).toFile(output);
