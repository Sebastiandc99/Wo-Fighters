...
test('all damage is scaled once to 60%, with resistance and 90-second rounds',()=>{
 const g=game();
 for(const kind of ['angel','primitivo','peluche'])for(const damage of [1,3,5,22,23,34,35]){
  ...
  const expected=Math.round(damage*.6*100000/g.run(`stats.${kind}.resistance`))/1000;
 ...
 assert.ok(g.run('cpu.health')>62);assert.equal(g.run('state'),'playing');
...
test('pause shows both selected fighters and matchup damage, with second-player keys only in versus',()=>{
 ...
 assert.ok(g.nodes.get('pausePowers1').innerHTML.includes('12%'));
